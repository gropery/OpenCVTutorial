# 参考文档 https://www.zhihu.com/question/270868590/answer/2577342534
    ...
    try:
        ...
        if defects is not None:
            ...
            for i in range(defects.shape[0]):  # calculate the angle
                ...
                c = np.sqrt((end[0] - start[0]) ** 2 + (end[1] - start[1]) ** 2)  # c^2 = x^2 + y^2 长度公式
                b = np.sqrt((far[0] - start[0]) ** 2 + (far[1] - start[1]) ** 2)
                a = np.sqrt((end[0] - far[0]) ** 2 + (end[1] - far[1]) ** 2)
                angle = np.arccos((a ** 2 + b ** 2 - c ** 2) / (2 * a * b))  #      cosine theorem
